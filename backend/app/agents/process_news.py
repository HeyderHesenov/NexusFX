"""AI emal dövrü — emal olunmamış xəbərləri AI ilə 4 dilə çevirir.
...
async def process_pending(limit: int = 12) -> dict[str, int]:
    """Emal olunmamış xəbərləri AI ilə emal edir. Sayğacları qaytarır."""
    ...
    # AI çağırışları paralel (sessiyadan kənar — təhlükəsiz).
...
async def main() -> None:
    if not has_primary():
        print("❌ LLM_PRIMARY_KEY yoxdur (.env yoxla).")
        return
    limit = int(sys.argv[1]) if len(sys.argv) > 1 else 12
    print(f"⏳ {limit} xəbər emal olunur (AI)…")
